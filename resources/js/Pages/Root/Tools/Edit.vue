    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 bg-background p-4">
            <div class="mb-6 flex items-center justify-between">
                <h1 class="animate-text-glow text-2xl font-bold text-glow">Editar Herramienta</h1>
                <CyberLink :href="route('root.tools.index')" variant="outline" size="md"> Volver a Herramientas </CyberLink>
            </div>

            <!-- Edit Tool Form -->
            <div class="glass-effect neon-border mx-auto w-full max-w-4xl rounded-xl p-6">
                <form @submit.prevent="submit" class="space-y-6">
                    <!-- Title -->
                    <div class="space-y-2">
                        <label for="title" class="block text-sm font-medium text-foreground/80">Título</label>
                        <AnimatedInputBorder id="title" v-model="form.title" placeholder="Nombre de la herramienta" required @input="generateSlug" />
                    ...
                    <div class="space-y-2">
                        <label for="slug" class="block text-sm font-medium text-foreground/80">
                            Slug (URL)
                            <span @click="generateSlug" class="ml-2 cursor-pointer text-xs text-primary transition-colors hover:text-primary/80">
                                (Generar desde título)
                            </span>
                        </label>
                        <AnimatedInputBorder id="slug" v-model="form.slug" placeholder="nombre-de-la-herramienta" required />
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <select
                                ...
                                class="w-full rounded-xl border border-white/10 bg-background/50 px-3 py-2 text-foreground transition-all duration-300 focus:border-cyan-400/30 focus:bg-background/70 focus:outline-none focus:ring-1 focus:ring-cyan-400/30"
                                required
                            >
                    ...
                    <div class="space-y-2">
                        <label for="excerpt" class="block text-sm font-medium text-foreground/80">
                            Extracto
                            <span class="ml-1 text-xs text-muted-foreground">(máx 500 caracteres)</span>
                        </label>
                        <textarea
                            id="excerpt"
                            ...
                            rows="3"
                            maxlength="500"
                            class="w-full rounded-xl border border-white/10 bg-background/50 px-3 py-2 text-foreground transition-all duration-300 placeholder:text-foreground/50 focus:border-cyan-400/30 focus:bg-background/70 focus:outline-none focus:ring-1 focus:ring-cyan-400/30"
                            placeholder="Descripción breve de la herramienta"
                        ></textarea>
                        <div class="text-xs text-muted-foreground">{{ form.excerpt?.length || 0 }}/500 caracteres</div>
                    ...
                    <div class="space-y-2">
                        <label for="featured_image" class="block text-sm font-medium text-foreground/80">
                            Imagen Principal
                            <span class="ml-1 text-xs text-muted-foreground">(JPG, PNG, WebP - máx 5MB)</span>
                        </label>

                        <!-- Current Image -->
                        <div v-if="currentImageUrl && !imagePreview" class="mt-2">
                            <p class="mb-2 text-xs text-muted-foreground">Imagen actual:</p>
                        ...
                        <input
                            ...
                            class="w-full rounded-xl border border-white/10 bg-background/50 px-3 py-2 text-foreground transition-all duration-300 file:mr-4 file:rounded-full file:border-0 file:bg-primary/10 file:px-4 file:py-1 file:text-sm file:font-semibold file:text-primary hover:file:bg-primary/20 focus:border-cyan-400/30 focus:outline-none focus:ring-1 focus:ring-cyan-400/30"
                        />
                        <p v-if="form.errors.featured_image" class="mt-1 text-sm text-red-500">{{ form.errors.featured_image }}</p>

                        <!-- New Image Preview -->
                        <div v-if="imagePreview" class="mt-2">
                            <p class="mb-2 text-xs text-muted-foreground">Nueva imagen:</p>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea
                            ...
                            rows="10"
                            class="w-full rounded-xl border border-white/10 bg-background/50 px-3 py-2 font-mono text-sm text-foreground transition-all duration-300 placeholder:text-foreground/50 focus:border-cyan-400/30 focus:bg-background/70 focus:outline-none focus:ring-1 focus:ring-cyan-400/30"
...
- Usa encabezados, listas, etc."
                    ...
                    <div class="space-y-4 border-t border-border/30 pt-6">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div class="space-y-2">
                                ...
                                <AnimatedInputBorder id="website_url" v-model="form.website_url" type="url" placeholder="https://ejemplo.com" />
                    ...
                    <div class="space-y-4 border-t border-border/30 pt-6">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="max-h-48 space-y-2 overflow-y-auto rounded-lg border border-white/10 p-3">
                                    <label
                                        v-for="category in categories"
                                        :key="category.id"
                                        class="flex items-center space-x-2 rounded p-1 transition-colors hover:bg-sidebar-accent/20"
                                    >
                                        <input
                                            type="checkbox"
                                            :value="category.id"
                        ...
                        <div class="space-y-4">
                            <div class="space-y-2">
                                <label for="meta_title" class="block text-sm font-medium text-foreground/80">
                                    Título SEO
                                    <span class="ml-1 text-xs text-muted-foreground">(máx 60 caracteres)</span>
                                </label>
                                <AnimatedInputBorder
                                    id="meta_title"
                                    ...
                                    maxlength="60"
                                />
                                <div class="text-xs text-muted-foreground">{{ form.meta_title?.length || 0 }}/60 caracteres</div>
                            ...
                            <div class="space-y-2">
                                <label for="meta_description" class="block text-sm font-medium text-foreground/80">
                                    Descripción SEO
                                    <span class="ml-1 text-xs text-muted-foreground">(máx 160 caracteres)</span>
                                </label>
                                <textarea
                                    id="meta_description"
                                ...
                                ></textarea>
                                <div class="text-xs text-muted-foreground">{{ form.meta_description?.length || 0 }}/160 caracteres</div>
                    ...
                    <div class="space-y-2 border-t border-border/30 pt-6">
                        ...
                        <AnimatedInputBorder id="published_at" type="datetime-local" v-model="form.published_at" />
                    ...
                    <div class="border-t border-border/30 pt-4">
                        <button
                            type="submit"
                            class="neon-border w-full rounded-lg border border-primary/30 bg-primary/10 px-4 py-2 font-medium text-primary-foreground transition-all duration-300 hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2"
...
import type { BreadcrumbItem, Category, Tag, Tool } from '@/types';
...
const form = useForm({
    ...
    published_at: props.tool.published_at ? new Date(props.tool.published_at).toISOString().slice(0, 16) : new Date().toISOString().slice(0, 16),
});
