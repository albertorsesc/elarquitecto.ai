<template>
    <div class="drop-zone-wrapper" :class="{ active: isDragging }" @click.stop>
        <Vue3Dropzone
            ...
            class="rounded-xl border-2 border-dashed border-white/20 transition-all duration-300 hover:border-primary/50"
            @error="handleError"
        ...
        >
            <template #title>
                <span class="text-sm font-medium text-foreground/80"> Arrastra y suelta tus {{ fileTypeLabel }} o haz click para seleccionar </span>
            </template>

            <template #description>
                <span class="text-xs text-foreground/60"> Supported formats: {{ formattedSupportedFormats }} (Max {{ maxFileSize }}MB) </span>
            </template>
            ...
            <template #button="{ fileInput }">
                <button
                    ...
                    class="mt-3 rounded-md bg-primary/20 px-4 py-2 text-sm text-primary-foreground transition-colors hover:bg-primary/30"
                >
                    {{ selectButtonText }}
                </button>
            </template>
            ...
            <template #placeholder-img>
                <div class="flex h-full w-full items-center justify-center">
                    <svg
                        ...
                        class="h-16 w-16 text-foreground/30"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                    >
            ...
            <template #preview="{ data, formatSize, removeFile }">
                <div class="group relative">
                    <img :src="data.src" class="h-48 w-full rounded-lg object-cover" />
                    <div class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity group-hover:opacity-100">
                        <button @click="removeFile(data)" class="rounded-full bg-destructive/90 p-2 text-white" type="button">
...
import Vue3Dropzone, { DropzoneError, DropzoneFileData } from '@jaxtheprime/vue3-dropzone';
