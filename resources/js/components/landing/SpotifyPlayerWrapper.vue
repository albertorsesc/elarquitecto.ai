...
const isSpotifyEnabled = ref(config.spotify.enabled);

onMounted(() => {
  // If Spotify is disabled via environment variable, don't even try to load it
  if (!isSpotifyEnabled.value) {
...
<template>
  <div>
    ...
    <!-- Use SpotifyPlayer if Pinia is available and Spotify is enabled -->
    <Suspense v-else-if="isPiniaAvailable && isSpotifyEnabled">
      <template #default>
        <SpotifyPlayer />
      </template>
      <template #fallback>
    ...
    <!-- Error state if Pinia is not available but Spotify is enabled -->
    <div v-else-if="!isPiniaAvailable && isSpotifyEnabled" class="fixed bottom-4 right-4 z-40">
      <div class="glass-effect overflow-hidden rounded-lg border border-white/10 bg-background/80 p-4 shadow-lg backdrop-blur-sm">
        <div class="flex items-center">
    ...
    <!-- Fallback to MusicPlayer when Spotify is disabled -->
    <div v-else-if="!isSpotifyEnabled">
      <MusicPlayer />
    </div>
  </div>
</template>