...
const classes = computed(() => {
    const baseClasses =
        'inline-flex items-center justify-center rounded-lg font-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50';
...
<template>
    <Link :href="href" :class="classes" :disabled="disabled">
        <div class="relative overflow-hidden">
            ...
            <div v-if="glow" class="absolute -inset-px opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                <div
                    class="absolute inset-0 rounded-md bg-gradient-to-r from-primary/50 via-cyan-400/50 to-secondary/50 opacity-0 backdrop-blur-sm group-hover:opacity-20"
                ></div>
                <div class="absolute inset-0 flex items-center justify-center">
                    <div
                        class="h-[1px] w-[70%] translate-y-[8px] transform animate-neon-slide-right-slow bg-gradient-to-r from-primary via-cyan-400 to-secondary blur-[2px]"
                    ></div>
                </div>
            </div>
        </div>
...
.neon-border::before {
    ...
    background: linear-gradient(90deg, hsl(var(--primary)), hsl(var(--cyan)), hsl(var(--secondary)));
