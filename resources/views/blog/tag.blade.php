...
@section('meta_description', 'Artículos con la etiqueta #' . $tag->name . ' en El Arquitecto A.I.')
...
@section('breadcrumbs')
    <div class="flex items-center gap-2 text-sm">
        ...
        <span class="text-foreground/40">/</span>
        <span class="text-foreground/80">#{{ $tag->name }}</span>
    </div>
@endsection

@section('content')
    <div class="mb-8">
        <h1 class="text-glow mb-4 text-3xl font-bold sm:text-4xl">#{{ $tag->name }}</h1>
        <p class="text-foreground/70">Artículos con la etiqueta #{{ $tag->name }}</p>
    </div>
    ...
    @else
        <div class="glass-effect rounded-xl p-8 text-center">
            <p class="text-foreground/70">No hay artículos con esta etiqueta.</p>
        </div>
    @endif
@endsection