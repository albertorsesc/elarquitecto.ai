...
@section('meta_description', 'Resultados de búsqueda para "' . $query . '" en El Arquitecto A.I.')
...
@section('breadcrumbs')
    <div class="flex items-center gap-2 text-sm">
        ...
        <span class="text-foreground/40">/</span>
        <span class="text-foreground/80">Búsqueda</span>
    </div>
@endsection

@section('content')
    <div class="mb-8">
        <h1 class="text-glow mb-4 text-3xl font-bold sm:text-4xl">Resultados de búsqueda</h1>
        <p class="text-foreground/70">Mostrando resultados para "<span class="text-primary">{{ $query }}</span>"</p>
    </div>
    ...
    @else
        <div class="glass-effect rounded-xl p-8 text-center">
            <p class="text-foreground/70">No se encontraron resultados para "<span class="text-primary">{{ $query }}</span>".</p>
            <p class="mt-4 text-sm text-foreground/50">Intenta con otras palabras o explora nuestras categorías.</p>

            <div class="mt-6">
                <a
                    href="{{ route('blog.index') }}"
                    class="neon-border rounded bg-primary/10 px-4 py-2 text-sm font-semibold text-primary transition-all hover:bg-primary/20"
                >
                    Volver al Blog
                </a>
            </div>
