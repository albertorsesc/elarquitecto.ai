...
<div class="group relative h-full overflow-hidden rounded-xl border border-cyan-400/20 glass-effect transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_25px_rgba(0,255,225,0.3)] hover:border-cyan-400/40">
    ...
    <div class="relative h-44 w-full overflow-hidden">
        ...
        <div class="absolute top-3 right-3 z-10">
            <div class="p-2 bg-yellow-500/20 backdrop-blur-md rounded-full border border-yellow-500/30" aria-label="Herramienta destacada">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-yellow-400" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div class="absolute bottom-3 left-3 z-10">
            <span class="px-2.5 py-1 text-xs rounded-full bg-black/80 backdrop-blur-sm text-cyan-300 border border-cyan-400/60 font-medium">
                {{ $categoryName }}
            </span>
        </div>
    ...
    <div class="flex flex-col gap-2.5 p-4">
        ...
        <div class="flex flex-wrap gap-1.5 mt-1" role="list" aria-label="Etiquetas">
            ...
            @if($tool->tags->count() > 3)
                <span class="text-xs px-2 py-0.5 rounded-md bg-cyan-500/10 text-cyan-400/80 border border-cyan-500/20" role="listitem">
                    +{{ $tool->tags->count() - 3 }}
                </span>
            @endif
        </div>
        ...
        <div class="mt-auto pt-3 flex items-center justify-between text-xs border-t border-cyan-400/10">
            <span class="flex items-center gap-2">
                @if($tool->website_url)
                <span class="flex items-center gap-1 text-cyan-400" aria-label="Sitio web disponible">
                    ...
                    <span class="sr-only">Sitio web:</span> Online
                </span>
                @endif
                @if($tool->documentation_url)
                <span class="flex items-center gap-1 text-purple-400" aria-label="Documentación disponible">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                    </svg>
                    <span class="sr-only">Documentación:</span> Docs
                </span>
                @endif
            </span>
            <time datetime="{{ $tool->published_at ?? $tool->created_at }}" class="text-cyan-500">{{ $date }}</time>
        </div>
        ...
        <a 
            ...
            aria-label="Explorar {{ $tool->title }}"
        >
